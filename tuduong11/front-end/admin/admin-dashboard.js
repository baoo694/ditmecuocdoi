...
import { getAllMedicalRecords } from '../chat/medicalRecordApi.js'
...
async function loadAdminMedicalRecords() {
  try {
    const token = localStorage.getItem('token')
    if (!token) return
    const data = await getAllMedicalRecords(token)
    const tbody = document.querySelector('#admin-medical-records-table tbody')
    tbody.innerHTML = ''
    ;(data.byPatient || []).forEach((record) => {
      // Nếu API trả về byPatient là mảng các record, cần sửa lại cho đúng
      // Nếu trả về records: [...], thì lặp qua data.records
    })
    // Nếu trả về records: [...]
    ;(data.records || []).forEach((record) => {
      ...
      tr.innerHTML = `
        ...
        <td class="table-cell-ellipsis" title="${record.diagnosis}">${
        record.diagnosis
      }</td>
        <td class="table-cell-ellipsis" title="${record.treatment}">${
        record.treatment || ''
      }</td>
        <td class="table-cell-ellipsis" title="${record.notes}">${
        record.notes || ''
      }</td>
        <td>${record.status || ''}</td>
      `
